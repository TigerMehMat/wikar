        ...
        this.sendMessage(options.title, options.body(timer), message, true, (msg) => {
            ...
            timerMessage.awaitReactions(reactionFilter, {max: 1, time: timer*60*1000, errors: ['time']})
                .then((res) => {
                    ...
                    this.sendMessage('Таймер отменен', 'Пользователь '+res.find((reac) => {return reac.me===true}).users.last().username+' отменил таймер "'+options.title+'"', timerMessage, false, () => {return;});
